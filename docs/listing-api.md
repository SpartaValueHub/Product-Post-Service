# Listing API (FE 연동 스펙)

Listing-Service 판매글 API 명세서입니다.  
식별자는 내부 PK가 아니라 **`listingUuid`** 를 사용합니다.
...
| Auth | Gateway 경유 시 JWT 검증 후 `X-Member-Uuid` 전달. Listing은 헤더로 판매자 식별 |
...
```json
{
  ...
  "fieldErrors": [
    { "field": "listingName", "code": "NotBlank", "message": "상품명은 필수입니다." }
  ]
}
```
...
로컬에서 Listing만 직접 호출할 때는 Swagger/Postman에 동일 헤더를 수동으로 넣는다.
...
| listingName | string | Y | 2~100자 |
| conditionGrade | string | Y | S / A / B / C |
| price | number | Y | `listing.policy.min-price` 이상 (기본 500000, 환경변수 `LISTING_MIN_PRICE`로 변경 가능) |
...
```json
{
  "listingUuid": "aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee",
